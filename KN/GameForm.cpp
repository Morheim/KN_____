...
void KN::GameForm::Update()
{
	...
	if (endGame) {
		...
		if (MessageBox::Show("Запуцстить новую игру? ", "Внимание", MessageBoxButtons::YesNo) == Windows::Forms::DialogResult::Yes) {
			NewGame();
		}
		return;
	}
...
void KN::GameForm::NewGame()
{
	...
	if (state_gamer == 1) {
		...
		MessageBox::Show("Ошибка генирации первоначального выбора игрока!", "Ошибка!");
		return;
	}
}
